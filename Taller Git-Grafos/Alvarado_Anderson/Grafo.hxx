...
template <class T, class C>
T Grafo<T, C>::obtenerVertice(ll indice)
{
...
template <class T, class C>
bool Grafo<T, C>::buscarArista(ll origen, ll destino)
{
...
template <class T, class C>
void Grafo<T, C>::prim(T inicio, vector<T> &nuevosV, vector<T> &pred, vector<C> &nuevasA)
{
...
template <class T, class C>
void Grafo<T, C>::menorPeso(vector<pair<int, int>> &vistos)
{
    pair<int, int> aux;
    vector<C> temp;
    C menorPeso = 100000;
...
template <class T, class C>
void Grafo<T, C>::dijkstra(T inicio, vector<T> &s, vector<T> &pred, vector<C> &dist)
{
    vector<T> q = vertices;
    for (int i = 0; i < numVertices; i++)
    {
        dist.push_back(numeric_limits<int>::max()-1);
    }
